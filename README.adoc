...
=== SSO login in C3
You can use any user defined in the IDP to do interactive login to Confluent Control Center.
...
=== Logging into Confluent Platform using the CLI

If you are on the same machine and are able to use a browser, you can run:
...
Follow the instructions to complete the login. Note: The token expires rather quickly. You might need to reauthenticate from time to time.

For simplicity, use a variable holding the kafka endpoint.

```console
export CONFLUENT_REST_URL=http://broker1:8090/kafka
```

Optionally: Get a token for authenticating to Confluent Platform: 

```confole
export TOKEN=$(confluent iam user describe -o json | jq -r '.authentication_token')
```

Access services with the token, i.e. kSQLdb (for simplicity, we run this in the ksqdb-server container where the `ksql` binary is installed already):
...
Querying the directory can be done on the command line like this:

```console
ldapsearch -H ldap://localhost:389 -D cn=admin,dc=demo,dc=confluent,dc=io -b dc=demo,dc=confluent,dc=io -x -w admin
```

For easier management, `phpldapamin` is made available, too. Just point your browser to this address: http://localhost:8090.
Credentials

* Login DN: cn=admin,dc=demo,dc=confluent,dc=io
* Password: admin
...
Produce some data in AVRO format:
