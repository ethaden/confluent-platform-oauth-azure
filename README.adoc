...
We just use `Access tokens` (you can optionally disable `ID tokens`) in the `Authentication` settings. Note, implicit flows are not required either.

If you want to use the `confluent` cli, make sure to allow "public public client flows" (under `Manage` -> `Authentication`).
...
Based on your customizations in the `.env` file, the `client`  EntraID user you have specified should have access to produce and consume messages from topics starting with `test`.
...
However, the superuser is allowed to do that:
...
docker compose exec broker1 kafka-topics \
    ...
    --create \
    --topic abcde \
    --command-config /etc/confluent/configs/superuser.properties
```
...
Use `kafka-avro-console-producer` to produce to a Kafka topic using Schema ID Validation:
...
kafka-avro-console-producer \
  ...
  --property bearer.auth.issuer.endpoint.url=$IDP_TOKEN_ENDPOINT  \
  --property bearer.auth.client.id=$CLIENT_APP_ID \
  --property bearer.auth.client.secret=$CLIENT_APP_SECRET \
  --property bearer.auth.scope=$AZURE_OAUTH_SCOPE  \
  --producer.config mount/client.properties \
  --topic test-avro \
  --property value.schema='{"type":"record","name":"Transaction","fields":[{"name":"id","type":"string"},{"name": "amount", "type": "double"}]}'
```

If you do not have the `kafka-avro-console-producer` locally, you can also run it from the `schema-registry` container (but expect to see lots of logs in the output):
...
Short form (here, the OIDC settings are passed using producer config):
...
Now, use `kafka-avro-console-consumer` to consume from the topic:
...
kafka-avro-console-consumer \
    ...
    --consumer.config mount/client.properties
```

If you do not have the `kafka-avro-console-consumer` locally, you can also run it from the `schema-registry` container (but expect to see lots of logs in the output):
...
=== Get a temporary (!) access token manually

All the endpoints are accessible using OAuth token.

For simplicity, set (or update) all required variables by running (in your shell):

```consolse
source helper/set_user_tokens.sh
```

If you would like to get an access token manually, this is what you'd need to run (the script does this for you already):
...
1. Create datagen source Connector (make sure you have sourced the `set_user_tokens.sh`first):

```console
curl --location 'broker1:8083/connectors' \
    ...
    \"config\": {
        ...
        \"quickstart\": \"users\",
        \"producer.override.sasl.oauthbearer.token.endpoint.url\": \"$IDP_TOKEN_ENDPOINT\",
        \"producer.override.sasl.jaas.config\": \"org.apache.kafka.common.security.oauthbearer.OAuthBearerLoginModule required clientId=\\\"$CLIENT_APP_ID\\\" clientSecret=\\\"$CLIENT_APP_SECRET\\\" scope=\\\"$AZURE_OAUTH_SCOPE\\\";\"
        }
}"
```

Obviously, using "$CLIENT_APP_ID" here is not recommended for production as it has superuser access. This is just for demo purposes.

Login to Confluent Control Center to validate messages being produced to topic `test-topic-datagen`.

Alternatively, you can start a consumer in the broker container to see the messages getting produced:
...
=== SSO login in C3
You can use any user defined in the IDP to do interactive login to Confluent Control Center: http://localhost:9021

=== SSO login in CLI 

[Download](https://docs.confluent.io/confluent-cli/current/install.html) the CLI binary that supports CLI SSO login for Confluent Platform.

You can use any user defined in IDP to do interactive login to Confluent Control Center.

You can login by specifying your MDS URL for CP cluster (if asked for a code just use the `user_code` which is already part of the URL).

```console
confluent login --url http://localhost:8090
```
Note, that we aren't using the suffix `/kafka` here (see below), as the login endpoint is part of the MDS service, not the Kafka REST Proxy.

For accessing REST Proxy endpoints of your Confluent Platform instance with the `confluent` CLI, you can set the following environment variable and skip the `--url` parameter in the commands:

```console
export CONFLUENT_REST_URL=http://localhost:8090/kafka
```

Add flag `--no-browser` if you are running this on a remote machine without browser access.

After successful login, you can now use the CLI to interact with Confluent Platform.
For example, list the topics configure in the the cluster:

```consolse
confluent kafka topic list --url $CONFLUENT_REST_URL/kafka
```
...
```consolse
confluent kafka topic create mytest --url $CONFLUENT_REST_URL/kafka
```

Optionally, you can get the access token issued by the `confluent` CLI:

```console
export TOKEN=$(confluent iam user describe -o json|jq -r .authentication_token)
```

This token can be used to access the REST API of Confluent Platform services, for example:

```console
curl 'http://broker1:8090/kafka/v3/clusters' --header 'Content-Type: application/json' --header "Authorization: Bearer $TOKEN"
```

This token also works for kSQLdb (for simplicity, we run this in the ksqdb-server container where the `ksql` binary is installed already):

```console
docker compose exec -ti ksqldb-server ksql http://ksqldb-server:8088 --token $TOKEN
```
...
The demo in subfolder `cluster-ldap-external-only-oidc` comes with an integrated OpenLDP server simulating active directory.

The directory is pre-populated with some users and groups:

* User: client, password: client
* User: client2, password: client2
* User client3, password client3

Querying the directory can also be done on the command line like this:

```console
ldapsearch -H ldap://localhost:1389 -D cn=admin,dc=demo,dc=confluent,dc=io -b dc=demo,dc=confluent,dc=io -x -w admin
```

For easier management, `phpldapamin` is made available, too. Just point your browser to this address: http://localhost:8090.
Note, due to the current setup, just these two users have access:

* Login DN: cn=admin,dc=demo,dc=confluent,dc=io, password: admin
* Login DN: cn=readonly,dc=confluent,dc=io, password: readonly

==== Producing and consuming using LDAP authentication

When using the configuration found in `cluster-ldap-external-only-oidc`, you can produce and consume messages using LDAP authentication and addition to using OIDC/OAuth.
Note, that the Kafka cluster uses OAuth for all internal communication in that case, too.

There is an additional Kafka endpoint with LDAP authentication enabled:

```
kafka:9095
```

All REST services support multiple authentication mechanisms, too.

A config file has been generated in the `mount` sub folder and made available in the broker container.
You can use the following commands to produce to and consume from a topic called test (or any other topic starting with `test`):

```console
docker compose exec broker1 kafka-console-producer \
    --bootstrap-server broker1:9095 \
    --topic test \
    --producer.config /etc/confluent/configs/client-ldap.properties

docker compose exec broker1 kafka-console-consumer \
    --bootstrap-server broker1:9095 \
    --from-beginning \
    --topic test \
    --consumer.config /etc/confluent/configs/client-ldap.properties
```

The REST Proxy services work with LDAP authentication, too. For example, querying the number of topics visible to the `client` user can be done like this:

```console
curl -u client:client --silent -X GET http://localhost:8090/kafka/v3/clusters/$CLUSTER_ID/topics
```

==== LDAP with Schema Registry

You can test access with curl. For example, you can query the existing subjects for the prefix `test*` like this:

```console
curl -u client:client --silent -X GET http://localhost:8081/subjects
```



Produce some data in AVRO format (note that the `kafka-avro-console-` commands do not use the `basic.auth.` values in the config file, so we need to pass them explicitly):

```console
kafka-avro-console-producer \
    --bootstrap-server broker1:9095 \
    --producer.config mount/client-ldap.properties \
    --topic test-avro \
    --property basic.auth.credentials.source=USER_INFO \
    --property basic.auth.user.info=client:client \
    --property value.schema='{"type":"record","name":"Transaction","fields":[{"name":"id","type":"string"},{"name": "amount", "type": "double"}]}'
```
...
You can use these JSON strings as input:
...
kafka-avro-console-consumer \
    ...
    --property basic.auth.credentials.source=USER_INFO \
    --property basic.auth.user.info=client:client \
    --consumer.config mount/client-ldap.properties
```

Again, the command can also be run from the `schema-registry` container:
