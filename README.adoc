...
```console
export CONFLUENT_REST_URL=http://localhost:8091/kafka
```
...
```console
curl 'http://broker1:8091/kafka/v3/clusters' --header 'Content-Type: application/json' --header "Authorization: Bearer $TOKEN"
```
...
```console
confluent login --url http://localhost:8091 --no-browser
```
...
```console
export CONFLUENT_REST_URL=http://broker1:8091/kafka
```
