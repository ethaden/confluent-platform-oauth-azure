...
```console
export TOKEN=$(confluent iam user describe -o json|jq -r .authentication_tok
en)
```
...
```console
curl 'http://broker1:8091/kafka/v3/clusters/test' --header 'Content-Type: application/json' --header "Authorization: Bearer $TOKEN"
```
